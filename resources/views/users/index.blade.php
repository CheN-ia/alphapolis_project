...
@if (auth()->id() == $user->id)
    ...
    <div>
        <h2>プロフィール</h2>
        <a href="{{ route('usersettings.user_show',  auth()->id()) }}">プロフィール設定</a>
    </div>
    ...
    <div>
        <h2>ブックマーク</h2>
        <a href="{{ route('users.bookmarks.index', auth()->id()) }}">ブックマーク一覧へ</a>
    </div>

    <div>
        <h2>投稿コメント</h2>
        <a href="{{ route('users.comment.index',  auth()->id()) }}">コメント一覧へ</a>
    </div>
